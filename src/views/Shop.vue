<template>
  <div class="shopPage" v-if="res" ref="shop">
    <div class="shopBox">
        <!-- 头部 -->
      <header class="header" >
        <div class="arrowLeft">
      ...
      <!-- tab -->
      <van-tabs title-active-color="rgb(49, 146, 239)" color="rgb(49, 146, 239)" :line-width="30" @click="isScroll">
        <!-- 商品区域 -->
        <van-tab title="商品">
          <goods :id="id"  />
        </van-tab>
...
export default {
  provide () {
    return {
      fatherScroll: this.parentScroll
    }
  },
  data () {
    return {
  ...
  computed: {
    parentScroll () {
      return this.$store.state.parentScroll
    }
  },
  components: {
    comment,
